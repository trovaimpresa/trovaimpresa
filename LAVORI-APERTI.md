...
## 🆕 IL 6 SETTEMBRE (notte) — i quattro problemi rimasti dopo lo smontaggio

### ✅ Chiusi
- ⛔ **LE GEMELLE ADESSO SI CONTROLLANO DA SOLE** · le due formule scritte in due posti (i costi della sicurezza in `compRiepilogo()` + vista `gest_computo_totali`; il filtro sulla fase del noleggio in `gest-riepilogo.js` + `gest-report.js`) avevano sopra un commento che diceva «si cambiano insieme». **Un commento non è una protezione.** Adesso c'è `prove-claude/banchi-fissi/gemelle/banco-gemelle.js` — →46← verdi, →10← rosse col `--sabota` — che **non guarda le parole delle due formule: le fa GIRARE sugli stessi dati e confronta i numeri** (→36← numeri su →12← computi finti, →10← fasi per il noleggio). Traduce la vista SQL in un conto eseguibile invece di riscriverla, e **se incontra SQL che non sa tradurre dice BANCO CIECO invece di restare verde** (provato: →4← cecità simulate, →4← fermate). Conta anche i posti dove quei conti esistono: se ne nasce un **terzo**, rosso. Nessun file del sito toccato (`prove-claude/` è in .gitignore)
- **IL GESTIONALE RISPONDE SUBITO DOPO UN SALVATAGGIO** · commit →67f5355← · misurato prima dal vivo: →14← query e →436 kB← per aver cambiato una riga, di cui **→354 kB← e →1.619 ms← per il PREZZARIO** — →500← voci riscaricate per riempire una tendina che ha **una riga sola** e sta in una pagina che non stai guardando. L'→84%← dell'attesa. Il database ci mette →9,6 ms←: il tempo è tutto nel viaggio. ⛔ **La memoria c'era già** (`ppTutte`, la stessa che usa `ppCerca()`, svuotata già nei →4← punti giusti da `gest-sal-prezzario.js`): `computoForm` era l'unico che la ignorava. Cura: **una riga**. Collaudo dal vivo: prima apertura →800 ms← (→354 kB←, una volta sola), poi →155← · →147← · →278← ms con **→0 kB←**. Banco: `banchi-fissi/velocita/banco-prezzario-memoria.js` — →12← verdi
- ⛔ **DUE DELLE TRE IPOTESI SULLA LENTEZZA ERANO SBAGLIATE, e solo il numero lo diceva** · «ridisegno intero invece che della sola riga?» → rifare tutto l'HTML della finestra costa **→4 ms←**
- **SI ENTRA NEL REPARTO ANCHE DA TASTIERA** · commit →0323ebc← · la scheda era un `<div>` nudo e il cestino dentro un `<button>`: su una schermata con una scheda sola si poteva **cancellare** un reparto ma non **entrarci**. Adesso `role="button"` + `tabindex="0"` (resta un `<div>`: un `<button>` dentro un `<button>` non è HTML valido) e **un** ascoltatore in un posto solo per Invio/barra spaziatrice, che salta i comandi veri così il cestino non parte due volte. Anello del fuoco blu del sito in `css/gestionale.css` (senza, Chrome ne disegna uno **arancione**). Banco: `banchi-fissi/tastiera/banco-tastiera.js` — →18← verdi, →6← rosse col `--sabota`, →8← situazioni provate
- **LE SPIEGAZIONI: →690← → →307← parole (−→55%←)** · commit →0e1e1c9← · giro completo di tutte le →26← sezioni, portato ad Alex **prima** di toccare i file (pagina pubblicata, testo di oggi / proposto / perché). **→3← tolte** (*Agenda·Da fare*, *Fornitori*, *Report*: dicono a parole quello che si vede), **→15← accorciate** (dentro ognuna c'era una cosa vera sepolta sotto la descrizione di quello che si vede), **→2← tenute intere** (*Richieste* e *Assistenza*: non sono spiegazioni, sono la voce di Alex). Le →7← sezioni che non avevano niente **restano nude**: la regola non è «una a testa», è «una solo dove serve». Banco: `banchi-fissi/spiegazioni/banco-spiegazioni.js` — →12← verdi, →2← rosse col `--sabota`

### ⏳ Aperti, nati stanotte
- ⚠️ **IL BANCO DELLE GEMELLE LEGGE LA COPIA SCRITTA DELLA VISTA, NON QUELLA VERA** · la vista `gest_computo_totali` sta dentro Supabase; nel progetto c'è solo `sql/capitolo-costi-sicurezza.sql`. Il →6 set← le due erano **identiche** (verificato con `pg_get_viewdef`), ma se un domani la vista si cambia dal pannello di Supabase e la copia non si aggiorna, **il banco continua a dire verde confrontando con una vista che non esiste più**. → Chi cambia quella vista aggiorna anche il file, sempre. È scritto a lettere grosse dentro il banco
- ⚠️ **Il confronto dice che le due formule sono d'accordo, non che hanno ragione** · se sbagliano insieme resta verde. Dentro ci sono →3← chiodi piantati a mano (→917,88←, ribasso →157,44←, totale →2.019,92←), ma sono →3←
- **Altri ~→200 ms← da recuperare dopo il salvataggio** · si rileggono anche **SAL** (→505 ms←) e **cronoprogramma** (→315 ms←), che stanno nelle pagine →5← e →6← del modulo, cioè roba che non hai davanti. ⚠️ Da fare piano: il cronoprogramma si era **già rotto una volta** perché non si riallineava (difetto →2← del 6 set pomeriggio). Serve un banco che provi che il capitolo nuovo si vede lo stesso
- **Quando CREI una lavorazione nuova il messaggio arriva dopo l'attesa**, non prima (`await compVoceForm(nuovoId)` e poi il toast). Sulla modifica invece arriva subito, dopo →167 ms←. È una riga
- ⛔ **→20← altri punti del gestionale sono comandi senza essere comandi** · lo stesso schema della scheda del reparto (un elemento che non è `<button>`/`<a>` con dentro un `data-action`) sta in →21← punti: ne è stato acceso **uno**. Gli altri →20← sono elencati in `banchi-fissi/tastiera/ancora-da-fare.json` e la prova →4← del banco diventa rossa **se ne nasce uno nuovo**. Adesso accenderne un altro costa due parole: la macchina per la tastiera c'è già
- **→108← note dentro i moduli, →2.675← parole** — quattro volte quelle delle sezioni. Lette tutte: sono quasi tutte **della specie buona** (perché il costo del personale entra nel margine, cosa fa il ribasso, le spese anticipate senza IVA, sotto →15← crediti non si opera). Non toccate. Ma è lì che c'è il grosso del testo, se un giorno si vuole guardare
- 🔴 **LE →8← ISCRIZIONI PERSE SONO ANCORA FERME** (nata il 6 set mattina, non chiusa) · in «Richieste dei clienti» ci sono →8← artigiani con nome, telefono e mestiere che volevano iscriversi. Una telefonata a testa: «ti registro io, gratis». **È ancora l'unica cosa che può portare iscritti questa settimana** — il gestionale adesso è più veloce e più solido, ma non si vende da solo

### ⛔ Il metodo, che ha funzionato tre volte su tre
Numeri scritti PRIMA · una cosa per volta · banco verde · banco `--sabota` rosso · `node tools/controllo-push.js` · push di Alex · collaudo dal vivo ricontando gli STESSI numeri. Mai due lavori in un push solo.
...
## 🆕 IL 6 SETTEMBRE (mattina) — admin, pannelli, profilo, modulo cerca

### ✅ Chiusi
- **Le tre cose che aspettavano Alex sono FATTE** (detto da lui il 6 set): l'email di conferma col logo e' tornata al suo posto · le →23← mail di richiesta conferma sono partite · il push e' andato (ultimo commit →55f2104←)
- **Admin — le scritte si leggono** · misura di partenza da →16px← a →18px← (`html { font-size }`, la leva generale: si cambia SOLO quel numero), →76← scritte a misura fissa ingrandite a mano, i due grigi da →#999/#555← a →#5f6b7a/#3f4652←. Barra laterale da →240px← a →310px←
- **Admin — via il grafico «Crescita iscritti»** e le →5← spunte sulle caselle in cima. ⏳ Il codice che lo disegnava (~→250← righe di JS + CSS) e' rimasto nel file, spento: se non lo rivuole si butta
- **Admin — caselle «Hanno aperto il sito ieri / oggi»** · vista `visite_giorno` (sql/vista-visite-giorno.sql) + `netlify/functions/admin-visite.js`. Ieri →44←, ▲→22%← sopra la media di →36←
- ⛔ **Admin — RIPARATO UN GUASTO VERO: `_set` era chiusa dentro `caricaImprese`** · Recensioni restava su «Caricamento...» per sempre e Segnalazioni scriveva «⚠️ Errore caricamento» pur non avendo nessun errore (il server rispondeva `{"success":true,"data":[]}`). Le due caselle sulla dashboard mostravano un trattino. Spostata fuori: riparate tutte e quattro le cose insieme
- **Admin — sezione «📞 Richieste dei clienti»** (ha preso il posto di «Preventivi») · legge `richieste_clienti`, che dal →21 lug← raccoglieva chi lascia il numero sui moduli «cerca» e che **nessuna pagina apriva**. Aggiunta alla lista bianca di `admin-dati.js`
- **Tolti →6← doppioni da `richieste_clienti`** (→20← righe → →14←) · stessa persona salvata →2←/→3← volte a →2-3← secondi di distanza, tutti fra il →27 lug← e il →2 ago←. La causa era gia' chiusa dal →2 ago← (commit →4d70df8←: tasto che si spegne + rifiuto lato server entro →2← minuti). Copia di sicurezza nella tabella `zz_doppioni_richieste_20260906`, sql in `sql/pulizia-doppioni-richieste-clienti.sql`
- **I 4 pannelli — certificazioni dentro il profilo**, via la casella dalla Dashboard
- **I 4 pannelli — via la pagina «👤 Il mio profilo»** · era la stessa roba di «Modifica profilo» ma da guardare e basta. Adesso il menu porta dritto al modulo
- **I 4 pannelli — via l'etichetta «⏳ In attesa di verifica»** (scelta di Alex: a tutti, sempre)
- **I 4 pannelli — scritte piu' grandi**, stessa leva dell'admin (→18px←) + →81/107/108/111← scritte fisse ingrandite
- **Profilo (`modifica-profilo.html`) rifatto** · titolo «✏️ Completa il tuo profilo» · una griglia sola per tutta la pagina (i blocchi con `display: contents`: prima ogni blocco chiudeva la riga a meta') · →6← titoletti inutili tolti · mappa richiusa · da →6.724px← a →5.034px← di altezza (**→-25%←**) · ogni sezione divisa da una linea grigia
- **Profilo — le caselle nascono bloccate** · si apre da leggere; ogni titolo ha a destra «✏️ Modifica» che sblocca SOLO il suo gruppo. →81← campi bloccati all'apertura
- ⛔ **DUE TRANELLI DA RICORDARE** · (1) con `display: contents` **ogni titolo e' il primo figlio del suo blocco**: la regola `.card-header:first-child`, messa per togliere la linea al primo, la spegneva a TUTTI (c'era in →2← punti). (2) una regola piu' specifica (`.type-section.visible .card-header`) si mangiava il margine dove la linea doveva stare. Sintomo identico in tutti e due i casi: «la linea non si vede»
- **Profilo — →8← campi nuovi**: prestazioni (una per riga → colonna `prestazioni`), orari, assicurazione RC (compagnia + scadenza), sopralluogo gratuito, consulenza online, aperto il sabato, e per le sole imprese SOA / ISO 9001 / white list antimafia. Erano tutte colonne che il database aveva gia' e che **nessuna pagina riempiva**
- **Profilo — →3← campi tolti**: terzo mestiere e terza professione (erano →4← modi per dire la stessa cosa), numero albo nascosto a chi non e' professionista
- **Profilo — gli anni di esperienza si calcolano** dall'anno di inizio: non invecchiano e non si contraddicono piu' (nel profilo di Alex dicevano →5← anni con anno →2009←, cioe' →17←). La colonna `anni_esperienza` resta e continua a riempirsi, perche' la leggono scheda pubblica, pannelli, admin e registrazioni
- ⛔ **Il modulo «cerca» adesso chiede PRIMA chi sei** (`cerca-imprese`, `cerca-artigiani`, `cerca-negozi`, `cerca-professionisti`, `professionisti`) · su →14← richieste vere **→8← erano artigiani che si proponevano** («Eseguo tutti i lavori», «sono J D Costruzioni», «muratore con partita IVA»). Il cartello giallo che diceva «questo form non serve a iscriversi» non lo leggeva nessuno. Adesso c'e' un bivio: **🔍 Cerco** apre il modulo · **🔧 Offro io questo servizio** porta alla registrazione gratuita. Il modulo non esiste finche' non si sceglie
- **Radiografia del pannello admin** · →24← voci aperte una per una sul sito vero: →21← funzionano, →2← erano rotte (riparate), →7← vuote a zero. Pagina pubblicata come artefatto

- ✅ **CHIUSA la voce →6← — «il noleggio prenotato contato come spesa»** · era vera: `gestionale-app.html` prendeva ogni noleggio con l'uscita nel mese **senza guardare la fase**, quindi un mezzo promesso ma ancora in piazzale abbassava l'utile per soldi mai usciti. Fasi: `prenotato` · `fuori` · `rientrato`. Adesso contano solo `fuori` e `rientrato`, e la stessa regola e' stata messa **anche nel Report** (riga →15263←): se i due filtri si scollano, Riepilogo e Report tornano a dire due utili diversi. Le righe vecchie senza `fase` continuano a contare. Banco: `prove-claude/banchi-fissi/noleggio/prova-nol.js` — →5← verdi. ⚠️ Oggi nel database non c'e' nessun noleggio «prenotato» (→2← fuori, →1← rientrato): il difetto non mordeva ancora, avrebbe morso al primo mezzo prenotato
- ✅ **CHIUSA la voce →7← — «le parole del cantiere»: era GIA' FATTA** il →5 set← notte e il quaderno aveva una riga vecchia. Il vocabolario `PAROLE` + `_p()` + `adattaParole()` in `gestionale-operatore.html` copre impresa/artigiano (predefinito), professionista («Su cosa hai lavorato — sopralluogo, rilievo, deposito in Comune») e negozio («Cosa hai fatto — scarico bancali, riordino magazzino»). In `gestionale-app.html` di «premiscelato» non c'e' traccia. La parola «cantiere» resta solo nei commenti del codice

- ✅ **CHIUSA la voce →5← (primo pezzo) — LA TIMBRATURA** · negli altri gestionali da cantiere (Fluida, Connecteam) la mattina l'operaio fa UNA cosa: preme «Entrata». Qui l'unico modo di segnare le ore era «Segna la giornata», che si compila a fine turno **a memoria** — e spesso non si compilava. Adesso in cima all'app c'è un pulsante grosso Entrata/Uscita, la striscia verde «Sei entrato alle 07:42 — 3h 18m» che sale da sola, e l'elenco della giornata col totale. **Funziona SENZA CAMPO**: la timbratura si scrive prima sul telefono (`localStorage`), si vede subito, e parte da sola quando torna la rete — si può anche chiudere l'app. Tabella `gest_timbrature` (→12← colonne, →4← regole RLS che ricalcano quelle dei rapportini: preposto e segretaria vedono tutti, l'operaio solo i suoi), indice unico su `(user_id, client_id)`: se la spedizione si ripete Postgres risponde →23505← e per noi vuol dire «era già arrivata», non un errore — senza quello una timbratura riprovata →3← volte diventava →3← entrate. Permesso: lo stesso dei rapportini. Banco: `prove-claude/banchi-fissi/operatore/banco-timbratura.js` — **→20← verdi**, e togliendo il riconoscimento del 23505 diventa rossa proprio la coda che non si svuota mai. ⚠️ **NON scrive dentro `gest_ore`**: se lo facesse le ore si sommerebbero a quelle scritte a mano in «Segna la giornata» e la busta paga verrebbe doppia. Per adesso è un registro a parte

- ✅ **LE ORE NEL GESTIONALE DEL TITOLARE** (il seguito della timbratura, chiesto da Alex il 6 set) · «Agenda operatore» adesso ha **due linguette**: «Da fare» (quello di prima) e «Ore fatte». Scelta di Alex: NON una voce nuova nel menu, che sarebbe stata la →25ª←. Dentro: mese avanti/indietro, →4← caselle in cima (ore timbrate · ore scritte a mano · giornate aperte · chi ha timbrato), una riga per persona che si apre sui giorni, e «⬇️ Scarica per Excel» (CSV con `;` e BOM: Excel italiano apre le colonne giuste e legge gli accenti). **Le ore timbrate e quelle scritte a mano stanno affiancate e NON si sommano mai** — sono due misure della stessa giornata, sommarle raddoppierebbe la busta paga. ⚠️ Niente filtro per reparto: `gest_ore` ha `mestiere_id`, `gest_timbrature` no (il telefono non chiede il reparto) — filtrarne una sola darebbe due numeri non confrontabili in silenzio, quindi si contano le ore di tutta l'azienda e la scritta in cima lo dice. **La giornata rimasta aperta** (entra e non timbra l'uscita: succederà spesso) si segnala in arancione e il titolare ci mette lui l'orario: si AGGIUNGE una riga uscita con `corretta_da`, la riga dell'operaio non si tocca mai — colonne nuove `corretta_da`, `corretta_il`, `quando_originale`. Chi la vede: solo il titolare (gestionale-app.html è già suo). Banco: `prove-claude/banchi-fissi/ore/banco-ore.js` — **→26← verdi**, e togliendo la protezione sul doppio tocco diventa rossa proprio quella (due entrate di fila rubavano mezz'ora all'operaio)

### ⏳ Aperti, nati oggi
- **Le →8← iscrizioni perse** · in «Richieste dei clienti» ci sono →8← artigiani con nome, telefono e mestiere che volevano iscriversi e sono finiti nel cassetto. Una telefonata a testa: «ti registro io, gratis». **E' la cosa di oggi che puo' portare iscritti questa settimana**
- **Il pannello admin sul telefono resta scomodo** · la barra laterale e' fissa a →310px← su →390px← di schermo. Serve un menu «a panino» che si apre e si chiude
- **Anni di esperienza del professionista** · li' non si calcola: manca l'anno di inizio attivita' nella sua sezione
- **Il codice del grafico «Crescita iscritti»** e' ancora dentro `admin.html`, spento e inoffensivo. Da buttare quando Alex e' sicuro di non rivolerlo
- **`_to_delete/`** · dentro ci sono →2← file di lock lasciati da git quando Claude lo lancia dal suo collegamento (non ha il permesso di cancellare sul computer di Alex). Da svuotare a mano
- ⚖️ **Solo →1← delle →14← richieste clienti ha il consenso registrato** (`consenso_at`): le altre →13← sono arrivate prima che il modulo iniziasse a segnarlo. Da far vedere a chi segue la privacy prima di richiamare gente di luglio
- ⚖️ **Da tenere d'occhio per una settimana**: se dopo il bivio arrivano meno richieste ma vere, ha funzionato. Se arriva zero di tutto, il problema non era il modulo — e' che sul sito arrivano artigiani e non clienti, e li' si cambia dove si fa pubblicita'
- ⏳ **Le →23← conferme**: le mail sono partite, ma al →6 set← mattina il database segna ancora →24← non confermate. E' normale: partire non vuol dire cliccare. Da ricontare fra →2-3← giorni
...
### ✅ Chiusi
- ⛔ **SERA: nel modello dell'email di conferma dentro Supabase e' finito il CODICE del banco `banco-logo-email.js`** (incollato per sbaglio, restava negli appunti). Salvato: chi si fosse iscritto avrebbe ricevuto pagine di codice **senza `{{ .ConfirmationURL }}`**, cioe' nessuno sarebbe riuscito a entrare. Scoperto per caso guardando una foto. Nessun danno: il →5 set← non si e' iscritto nessuno, e fino al →4 set← le conferme arrivavano nello stesso minuto. **RIMESSO A POSTO** con `Reset template`: adesso c'e' l'email standard di Supabase — brutta e in inglese, ma col link giusto. ⏳ **DA FINIRE**: rimettere l'email col logo, che sta in `docs/email-conferma-iscrizione.html` (basta `node tools/modello-email.js --scrivi`)
- **`tools/modello-email.js`** · legge e riscrive il modello dell'email di conferma da riga di comando (Management API, chiave `sbp_` in `$env:SUPABASE_ACCESS_TOKEN`). Prima di scrivere controlla che sia un'email vera: se il link di conferma non compare →2← volte, o il testo comincia con `#!`, **si rifiuta**. Fa sempre una copia di quello che c'era in `docs/`. Banco: `prove-claude/banchi-fissi/conferme/banco-modello-email.js` — →8← verdi, →3← rossi col controllo spento. ⏳ Alex non ha ancora creato la chiave
- **Il tetto delle mail all'ora NON era un problema**: misurato, e' →30← email/ora con l'SMTP suo (Resend, `noreply@trovaimpresa.com`), e le mail da mandare sono →23←. Passano tutte in un colpo. La protezione messa in `rimanda-conferme.js` resta come rete di sicurezza, ma non serviva
- **Il programma delle conferme adesso regge il tetto di mail all'ora** · Supabase accetta solo un tot di mail di conferma per ora; superato il tetto risponde →429←. Prima il programma scriveva ERRORE accanto alla persona e passava oltre: quella persona restava fuori e nessuno se ne accorgeva. Adesso un →429← non e' un errore ma un «riprova»: aspetta →1←, →5←, →15←, →30←, →60← minuti e ritenta con la stessa persona. E si segna chi ha gia' ricevuto in `tools/conferme-mandate.txt` (in .gitignore: dentro ci sono le email degli iscritti), cosi' se si ferma a meta' e lo rilanci riparte da dove era e a nessuno arrivano due mail uguali. Aggiunte due parole: `--quante 3` (ne prova solo 3) e `--tutti`. Banco: `prove-claude/banchi-fissi/conferme/banco-tetto-mail.js` — →7← verdi; col file di prima →6← rossi. ⏳ **Manca solo**: Alex approva il testo dell'email (Supabase → Authentication → Emails → Confirm signup) e legge se l'SMTP e' suo o quello incluso, poi si lancia
- **Ricontati i numeri dal database**: →117← vere · →93← confermate · →24← no · →1← doppione («ms gruop», stesso telefono della →163←) → **→23← mail**. Controllato in piu' che nessuno dei →24← abbia gia' confermato di nascosto: nessuna mail sprecata. ⚠️ Il massimo di iscritti VERI in un giorno e' →6← (→4 ago← e →22 lug←): i →7← del →5 set← erano gli account finti del collaudo

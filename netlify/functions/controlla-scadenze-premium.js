...
//
// Gira ogni mattina alle 9 e manda la mail "il tuo Gestionale (in regalo) scade
// fra 7 giorni" a chi ha l'abbonamento gratuito (reverse trial) in scadenza.
// È il messaggio che converte i regali in abbonamenti veri: senza questo avviso
// l'abbonamento scade in silenzio (il cron lo declassa a Free) e quasi nessuno
// torna a pagare.
//
...
// prezzi archiviati su Stripe dal 30 agosto. I veri sono 29/249 (Gestionale) e
// 39/349 (Gestionale AI). Le prime scadenze sono il 19 ottobre, quindi il 12
...
// se un'email nomina un prezzo del Gestionale che non esiste in prezzi.html.
//
...
// Gestionale): preventivi col PDF, fatture per lo SDI, lavori e cantieri,
...
// i 39€ del Gestionale AI aggiungono solo l'aiuto dell'AI dentro al gestionale.
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;margin:0;padding:24px 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 2px 12px rgba(10,42,77,0.08);">
        ...
        <tr><td style="background:#7b1fa2;padding:13px 32px;text-align:center;color:#ffffff;font-size:15px;font-weight:700;">⏳ Il tuo Gestionale scade fra 7 giorni</td></tr>
        <tr><td style="padding:32px;color:#1a2733;font-size:15px;line-height:1.65;">
          <p style="margin:0 0 16px;">${saluto}</p>
          <p style="margin:0 0 16px;">ti scrivo perché i <strong>3 mesi</strong> che ti ho attivato in regalo stanno per terminare: il tuo <strong>Gestionale</strong> resta attivo fino al <strong>${esc(dataItaliana(scadenza))}</strong>.</p>
          ...
          <p style="margin:0 0 10px;">Detto questo: con il Gestionale non paghi la visibilità, paghi <strong>il gestionale</strong>. È un programma per tenere in ordine il lavoro, pensato per le imprese edili e per gli artigiani:</p>
          ...
          <p style="margin:0 0 16px;">Il Gestionale costa <strong>€29 al mese</strong> oppure <strong>€249 all'anno</strong>. Dentro ci sono anche la maggiore visibilità sul sito, la priorità nei risultati di ricerca, la visibilità oltre la tua città e le foto e i video dei lavori senza limiti.</p>
          ...
          <p style="margin:0 0 16px;">Questo è il <strong>Gestionale AI</strong>: <strong>€39 al mese</strong> (o €349 all'anno), cioè 10 euro in più del Gestionale, con 300 messaggi al mese.</p>
...
const handler = async function () {
  ...
  try {
    // Regali in scadenza ESATTAMENTE fra 7 giorni (finestra di 1 giorno).
    ...
    for (const imp of inScadenza || []) {
      ...
      const ok = await inviaEmail(
        imp.email,
        '⏳ Il tuo Gestionale TrovaImpresa scade fra 7 giorni',
    ...
    if (risultato.avvisi > 0) {
      await inviaEmail(
        ...
        '🔔 Abbonamenti in scadenza: ' + risultato.avvisi + ' avvisi inviati',
        '<p>Ho inviato ' + risultato.avvisi + ' email "il Gestionale scade fra 7 giorni".</p>'
